...
from rmq.utils import Task, TaskObserver, RMQConstants, RMQDefaultOptions, TaskStatusCodes
from rmq.utils.decorators import rmq_callback, rmq_errback, call_once
...
class RPCTaskConsumer(object):
    ...
    def spider_opened(self, spider):
        ...
        self.completion_strategy = getattr(self.__spider, 'completion_strategy',
    ...
    def on_request_scheduled(self, request, spider):
        if self.delivery_tag_meta_key in request.meta.keys() and request.meta.get('retry_times') is None:
    ...
    def on_errback_completed(self, failure=None, spider=None, delivery_tag=None):
        if failure is not None and spider is not None:
            delivery_tag = failure.request.meta.get(self.delivery_tag_meta_key, None) \
                if delivery_tag is None else delivery_tag
    ...
    def on_spider_error(self, failure, response, spider):
        ...
        if delivery_tag is not None:
            hardware_errors = [HttpError, TunnelError, TimeoutError, TCPTimedOutError, DNSLookupError]
    ...
    def on_item_scheduled(self, response, spider, delivery_tag):
        if response is not None and spider is not None:
            ...
            if not current_task and self.completion_strategy == RPCTaskConsumer.CompletionStrategies.WEAK_ITEMS_BASED:
                return
    ...
    def _check_is_completed(self, spider=None, delivery_tag=None):
        ...
        if delivery_tag is not None and spider is not None:
            ...
            if self.completion_strategy == RPCTaskConsumer.CompletionStrategies.REQUESTS_BASED:
                ...
                if is_completed:
                    if current_task.status != TaskStatusCodes.ERROR:
                        if current_task.success_responses == 0 \
            ...
            elif self.completion_strategy in [RPCTaskConsumer.CompletionStrategies.STRONG_ITEMS_BASED,
                ...
                if current_task.reply_to is not None:
                    ...
                    if isinstance(self.rmq_connection.connection, pika.SelectConnection):
                        cb = functools.partial(
                            self.rmq_connection.publish_message, message=json.dumps(payload),
    ...
    def _validate_spider_has_decorators(self):
        ...
        spider_method_list = [func for func in dir(self.__spider) if callable(getattr(self.__spider, func))
