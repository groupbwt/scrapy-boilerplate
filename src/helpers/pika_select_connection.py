...
class PikaSelectConnection:
    ...
    def __init__(self, queue_name: str, callback: Callable, is_consumer: bool, options: dict = None, settings=None):
        """
        ...
        self.OPTIONS['enable_delivery_confirmations'] = options.get('enable_delivery_confirmations',
                                                                    self.DEFAULT_OPTIONS['enable_delivery_confirmations'])
        self.OPTIONS['prefetch_count'] = options.get('prefetch_count', self.DEFAULT_OPTIONS['prefetch_count'])
    ...
    def on_exchange_declare_ok_callback(self, _unused_frame):
        ...
        self.channel.queue_declare(queue=self.queue_name, callback=self.on_queue_declare_ok_callback, durable=True)
    ...
    def on_bind_ok_callback(self, _unused_frame):
        ...
        self.channel.basic_qos(prefetch_count=self.OPTIONS['prefetch_count'], callback=self.on_basic_qos_ok_callback)
    ...
    def start_consuming(self):
        ...
        self.consumer_tag = self.channel.basic_consume(
            ...
            on_message_callback=self.on_message_consume_callback
        )
    ...
    def on_cancel_ok_callback(self, _unused_frame):
        self.is_consuming = False
        self.logger.info(f'RabbitMQ acknowledged the cancellation of the consumer: {self.consumer_tag}')
        self.close_channel()
